...
while ($Row = $this->Context->Database->GetRow($this->DiscussionData)) {
    ...
    // Template de discussion spécifique
    if ($Discussion->Type && is_readable(ThemeFilePath($this->Context->Configuration, 'discussion_'.$Discussion->Type.'.php'))) {
        $ThemeFilePath = ThemeFilePath($this->Context->Configuration, 'discussion_'.$Discussion->Type.'.php');
    } else {
