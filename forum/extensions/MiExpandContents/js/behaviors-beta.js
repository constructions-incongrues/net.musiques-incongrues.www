jQuery(document).ready(function($) {
	...
	$.expr[':'].image = function(obj) {
		...
		if (href != undefined) {
			match = href.match(regexes.images);
		}
		return match;
	};

	$.expr[':'].video = function(obj) {
		var match = false;
		var href = $(obj).attr('href');
		if (href != undefined) {
			match = href.match(regexes.video);
		}
		return match;
	};
	
	$.expr[':'].audio = function(obj) {
		var match = false;
		var href = $(obj).attr('href');
		if (href != undefined) {
			match = href.match(regexes.audio);
		}
		return match;
	};
	...
	$('div#ContentBody a:audio').embedly({
		maxWidth: 450,
		method: 'replace'
	});
